...
class BasicBlock1D(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None, groups=1, dropout_rate=0.25, **kwargs):
        ...
        self.conv1 = nn.Conv1d(in_channels=in_channel, out_channels=out_channel,
                               kernel_size=5, stride=stride, padding=2, bias=False, groups=groups)
        self.bn1 = nn.BatchNorm1d(out_channel)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv1d(in_channels=out_channel, out_channels=out_channel,
                               kernel_size=5, stride=1, padding=2, bias=False, groups=groups)
...
class TFMS_CNN_kernel5(nn.Module):
    def __init__(self, num_classes=10, dropout_rate=0.25, groups=16):
        super(TFMS_CNN_kernel5, self).__init__()
    ...
    def _build_branch(self, block, num_blocks, kernel_size, initial_filters, dropout_rate):
        ...
        layers.append(
            nn.Sequential(
                nn.Conv1d(in_channels, initial_filters, kernel_size, padding=2, stride=2),
        ...
        for i in range(num_blocks):
            ...
            downsample = nn.Sequential(
                nn.Conv1d(initial_filters, initial_filters * 2, kernel_size=kernel_size, padding=2, stride=2, bias=False, groups=self.groups),
