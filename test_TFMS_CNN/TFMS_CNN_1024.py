...
class BasicBlock1D(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, kernel_size=7, downsample=None, groups=1, dropout_rate=0.25, **kwargs):
        super(BasicBlock1D, self).__init__()
        self.conv1 = nn.Conv1d(in_channels=in_channel, out_channels=out_channel,
                               kernel_size=kernel_size, stride=stride, padding=3, bias=False, groups=groups)
        self.bn1 = nn.BatchNorm1d(out_channel)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv1d(in_channels=out_channel, out_channels=out_channel,
                               kernel_size=kernel_size, stride=1, padding=3, bias=False, groups=groups)
...
class TFMS_CNN_1024(nn.Module):
    def __init__(self, num_classes=10, dropout_rate=0.25, groups=16):
        super(TFMS_CNN_1024, self).__init__()
    ...
    def _build_branch(self, block, num_blocks, kernel_size, initial_filters, dropout_rate):
        ...
        for i in range(num_blocks):
            ...
            layers.append(block(initial_filters, initial_filters * 2, stride=2, kernel_size=kernel_size, downsample=downsample, groups=self.groups, dropout_rate=dropout_rate))
            layers.append(block(in_channel=initial_filters * 2, out_channel=initial_filters * 2, kernel_size=kernel_size, groups=self.groups))
            initial_filters *= 2
