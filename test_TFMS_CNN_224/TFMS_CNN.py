...
class BasicBlock1D(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, stride=1, kernel_size=7, padding=3, downsample=None, groups=1, dropout_rate=0.25, **kwargs):
        super(BasicBlock1D, self).__init__()
        self.conv1 = nn.Conv1d(in_channels=in_channel, out_channels=out_channel,
                               kernel_size=kernel_size, stride=stride, padding=padding, bias=False, groups=groups)
        self.bn1 = nn.BatchNorm1d(out_channel)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv1d(in_channels=out_channel, out_channels=out_channel,
                               kernel_size=kernel_size, stride=1, padding=padding, bias=False, groups=groups)
...
class TFMS_CNN_Case3(nn.Module):
    def __init__(self, num_classes=10, dropout_rate=0.25, groups=16):
        ...
        # 频域分支
        self.freq_branch = self._build_branch(BasicBlock1D, num_blocks, kernel_size, padding, initial_filters, dropout_rate)
        
        # 全连接层
        self.fc_layer = nn.Sequential(
            nn.Linear(initial_filters * (2**3), 64),  # 每个分支的最终输出特征连接后输入到全连接层
            nn.ReLU(),
    ...
    def _build_branch(self, block, num_blocks, kernel_size, padding, initial_filters, dropout_rate):
        layers = []
        ...
        layers.append(
            nn.Sequential(
                nn.Conv1d(in_channels, initial_filters, kernel_size, padding=padding, stride=2),
        ...
        for i in range(num_blocks):
            ...
            downsample = nn.Sequential(
                nn.Conv1d(initial_filters, initial_filters * 2, kernel_size=kernel_size, padding=padding, stride=2, bias=False, groups=self.groups),
                nn.BatchNorm1d(initial_filters * 2)
            )
            layers.append(block(initial_filters, initial_filters * 2, stride=2, kernel_size=kernel_size, padding=padding, downsample=downsample, groups=self.groups, dropout_rate=dropout_rate))
            layers.append(block(in_channel=initial_filters * 2, out_channel=initial_filters * 2, kernel_size=kernel_size, padding=padding, groups=self.groups))
            initial_filters *= 2
...
# summary(model, input_size=(1, 224))
# inputs = torch.randn(1, 1, 224).to(device)
